...
vipro = LINETCR.LINE()
#vipro.login(qr=True)
vipro.login(token='ExSrHdItuPGBk72nnvu0.AHNTPW3AYambAplUcDVQea.gIUyvaGWthXxQqEtv03ATfNciHRo1aiq0SLaJiMG5HA=')
vipro.loginResult()
...
╠═════════════════════════
║             💠 By : Nurdin 💠
║    〽️ line.me/ti/p/~nurd1nsunzurion 〽️
╚═════════════════════════
"""
...
╚═════════════════════════
"""
vip="u6a19eda70ddac8030168f2ee09dbea10"
...
╠═════════════════════════
║             💠 By : Nurdin 💠
║    〽️line.me/ti/p/~nurd1nsunzurion〽️
╚═════════════════════════
"""
...
Bots=[mid]
Creator=["u6a19eda70ddac8030168f2ee09dbea10"]
...
wait = {
    ...
    'timeline':True,
    "Timeline":True,
    "comment":"Bot Auto Like ©By : Nurdin\nContact Me : 👉 line.me/ti/p/~nurd1nsunzurion ",    
    "commentOn":True,
    "commentBlack":{},
...
#                            Name = summon(op.param2)
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention2"] == True:          
                    contact = vipro.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["Aduh Kak Di Bilang Jangan Di Tag Dulu Lagi Kojom Sama Ica Nih kak 😊","Nggak Usah Tag-Tag! Kalo Penting Langsung Pc Aja","Woii " + cName + " Jangan Ngetag, Lagi Asik Sama Ica😘😘"]
                    ret_ = random.choice(balas)
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                    mentionees = mention['MENTIONEES']
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention3"] == True:          
                    contact = vipro.getContact(msg.from_)
                    cName = contact.displayName
                    balas = ["Hayy Kak " + cName + ", Jangan Kebanyakan Ngetag Nanti Suka Loh 😂😂"]
                    balas1 = "Ini Foto Yang Ngetag Saya Tadi. . ."
                    ret_ = random.choice(balas)
                    image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
                    name = re.findall(r'@(\w+)', msg.text)
                    mention = ast.literal_eval(msg.contentMetadata['MENTION'])
		...
		if wait["Simi"] == True: md+="╠➩✔️ Simisimi : On\n"
                ...
                msg.contentType = 9
                msg.contentMetadata={'PRDID': '3.18.31-perf-g459b3d5-343863-g0f74f56'} 
                                    'PRDTYPE': 'THEME',
                                    'MSGTPL': '8'}
                msg.text = None
                vipro.sendMessage(msg)
		...
		if midd not in admin:
		    ...
		    vipro.sendText(msg.to,"Admin Detected")
                ...
                midd = "   image = "u6a19eda70ddac8030168f2ee09dbea10"
                vipro.inviteIntoGroup(msg.to,[midd])
